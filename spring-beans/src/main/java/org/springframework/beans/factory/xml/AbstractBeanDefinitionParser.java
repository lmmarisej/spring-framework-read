...
public abstract class AbstractBeanDefinitionParser implements BeanDefinitionParser {
	...
	public final BeanDefinition parse(Element element, ParserContext parserContext) {
		AbstractBeanDefinition definition = parseInternal(element, parserContext);		// 解析 Element 得到 BeanDefinition 对象
		if (definition != null && !parserContext.isNested()) {
			try {
				String id = resolveId(element, definition, parserContext);		// 解析标签的 id
				...
				if (shouldParseNameAsAliases()) {
					String name = element.getAttribute(NAME_ATTRIBUTE);					// 标签的 name 属性
				...
				registerBeanDefinition(holder, parserContext.getRegistry());				// 注册 BeanDefinition
				if (shouldFireEvents()) {				// 是否需要出发事件
					BeanComponentDefinition componentDefinition = new BeanComponentDefinition(holder);	// 组件注册事件
