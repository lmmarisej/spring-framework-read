...
public abstract class AbstractPlatformTransactionManager
	 ...
	 * the underlying transaction infrastructure's default timeout. 获取默认超时时间
	 */
	 ...
	 * PROPAGATION_REQUIRED or PROPAGATION_SUPPORTS encountering an existing transaction), this
	 * outer transaction's characteristics will apply even to the inner transaction scope.
	 * Validation will detect incompatible isolation level and read-only settings on the inner
	 * transaction definition and reject participation accordingly through throwing a corresponding
	 * exception.
	 * <p>Default is "false", leniently ignoring inner transaction settings,
	 * simply overriding them with the outer transaction's characteristics. Switch this flag to
	 ...
	 * transaction is a rollback: The transaction originator <i>cannot</i> make the transaction
	 * commit anymore.
	 ...
	 * is a "nested transaction", where the global transaction can be rolled back to a savepoint
	 * taken at the beginning of the subtransaction. PROPAGATION_NESTED provides exactly those
	 * semantics; however, it will only work when nested transaction support is available. This is
	 * the case with DataSourceTransactionManager, but not with JtaTransactionManager.
	 *
	 ...
	 * outermost transaction boundary. Switch this flag on to cause an UnexpectedRollbackException
	 * as early as the global rollback-only marker has been first detected, even from within an
	 * inner transaction boundary.
	 ...
	 * UnexpectedRollbackException at the outermost transaction boundary. This allows, for example,
	 * to continue unit tests even after an operation failed and the transaction will never be
	 * completed. All transaction managers will only fail earlier if this flag has explicitly been
	 * set to "true".
	 *
	...
	private TransactionStatus handleExistingTransaction(
		...
		if (isValidateExistingTransaction()) {
			...
			if (definition.getIsolationLevel() != TransactionDefinition.ISOLATION_DEFAULT) {
				...
				// 当前事务隔离级别不为空 或者当前事务隔离级别和事务定义中的隔离级别不相同
	...
	protected final SuspendedResourcesHolder suspend(@Nullable Object transaction)
		...
		if (TransactionSynchronizationManager.isSynchronizationActive()) {
			...
			try {
				...
				// 在线程中保存与事物处理有关的信息，并重置线程中相关ThreadLocal变量
	...
	protected final void resume(@Nullable Object transaction,
		...
		if (resourcesHolder != null) {
			...
			if (suspendedSynchronizations != null) {
				TransactionSynchronizationManager
						.setActualTransactionActive(resourcesHolder.wasActive);
				TransactionSynchronizationManager
						.setCurrentTransactionIsolationLevel(resourcesHolder.isolationLevel);
				TransactionSynchronizationManager
						.setCurrentTransactionReadOnly(resourcesHolder.readOnly);
	 ...
	 * @see AbstractPlatformTransactionManager#prepareForCommit(org.springframework.transaction.support.DefaultTransactionStatus)
	 * @see AbstractPlatformTransactionManager#triggerBeforeCommit(org.springframework.transaction.support.DefaultTransactionStatus)
	 * @see AbstractPlatformTransactionManager#triggerBeforeCompletion(org.springframework.transaction.support.DefaultTransactionStatus)
	 * <b>后置方法</b>
	 * @see AbstractPlatformTransactionManager#triggerAfterCompletion(org.springframework.transaction.support.DefaultTransactionStatus,
	 * int)
	 * @see AbstractPlatformTransactionManager#triggerAfterCommit(org.springframework.transaction.support.DefaultTransactionStatus)
	 * @see AbstractPlatformTransactionManager#cleanupAfterCompletion(org.springframework.transaction.support.DefaultTransactionStatus)
	 * <p>
	 ...
	 * manager implementation, carrying corresponding transaction state in a modifiable fashion.
	 * This object will be passed into the other template methods (e.g. doBegin and doCommit),
	 * either directly or as part of a DefaultTransactionStatus instance.
	 ...
	 *                                                                          support is not
	 *                                                                          available
	 * @throws TransactionException                                             in case of lookup or
	 *                                                                          system errors
	 * @see #doBegin
	 * @see #doCommit
	 * @see #doRollback
	 ...
	 * SavepointManager interface, a NestedTransactionNotSupportedException will be thrown. Else,
	 * the SavepointManager will be asked to create a new savepoint to demarcate the start of the
	 * nested transaction.
	 ...
	 * {@code useSavepointForNestedTransaction()} returns "false", this method will be called to
	 * start a nested transaction when necessary. In such a context, there will be an active
	 * transaction: The implementation of this method has to detect this and start an appropriate
	 * nested transaction.
	 *
	 ...
	 * @throws TransactionException                                                   in case of
	 *                                                                                creation or
	 *                                                                                system errors
	 ...
	 * doResume)
	 * @throws org.springframework.transaction.TransactionSuspensionNotSupportedException if suspending
	 *                                                                                    is not supported
	 *                                                                                    by the transaction
	 *                                                                                    manager
	 *                                                                                    implementation
	 ...
	 * via the TransactionStatus, but only to the transaction itself being marked as rollback-only
	 ...
	 * itself. This should not be the typical case; it is mainly checked to cover misbehaving JTA
	 * providers that silently roll back even when the rollback has not been requested by the
	 * calling code.
	 *
	 * @see #doCommit
	 ...
	 * assuming that participating in existing transactions is generally not supported. Subclasses
